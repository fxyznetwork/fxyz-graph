/**
 * tm #1003 — sliceEdgeId must stay byte-compatible with the
 * @fxyz/graph-contract EdgeId grammar (refs.ts makeEdgeId), so the v1
 * contract-native slice swap is a rename, not a value migration.
 * The contract is a devDependency here on purpose: parity is a test-time
 * concern; graph-layout's v0 identity space carries no runtime contract dep.
 */
import { type GraphRef, makeEdgeId } from "@fxyz/graph-contract";
import { sliceEdgeId } from "../source/edge-id";

describe("sliceEdgeId (tm #1003)", () => {
	it("matches the contract EdgeId grammar exactly", () => {
		// Contract endpoints are GraphRefs; v0 slice keys stand in for them.
